...
export class LivreComponent implements OnInit {
  ...
  httpDateYear = 'latest';
  listpays = ['USD', 'EUR', 'JPY', 'ILS', 'CAD', 'CHF', 'MXN', 'RUB', 'CNY'];
  ...
  lien(e) {
    ...
    switch(e.target.value){
      case'1':
      this.url = this.httpBase.concat('base=GBP&symbols=USD');
      this.nom = "Le Dollar-USA";
      this.symbol = " $";
      break;
      case'2':
      this.url = this.httpBase.concat('base=GBP&symbols=GBP');
      this.nom = "Le Dollar-Canadien";
      this.symbol = " $C";
      break;
      case'3':
      this.url = this.httpBase.concat('base=GBP&symbols=JPY');
      this.nom = "Le Yen";
      this.symbol = " ¥";
      break;
      case'4':
      this.url = this.httpBase.concat('base=GBP&symbols=ILS');
      this.nom = "Le Shekel";
      this.symbol = " ₪";
      break;
      case'5':
      this.url = this.httpBase.concat('base=GBP&symbols=EUR');
      this.nom = "L'Euro'";
      this.symbol = " €";
      break;
      case'6':
      this.url = this.httpBase.concat('base=GBP&symbols=CHF');
      this.nom = "Le Franc-Suisse";
      this.symbol = " CHF";
      break;
      case'7':
      this.url = this.httpBase.concat('base=GBP&symbols=MXN');
      this.nom = "Le Peso";
      this.symbol = " MXN";
      break;
      case'8':
      this.url = this.httpBase.concat('base=GBP&symbols=RUB');
      this.nom = "Le Rouble";
      this.symbol = " ₽";
      break;
      case'9':
      this.url = this.httpBase.concat('base=GBP&symbols=CNY');
      this.nom = "Le Yuan";
      this.symbol = " ¥";
      break;
     }
...
get12value(): void{
  ...
  for (let index = 0; index < 12; index++)
  {
    for (let index1 = 0; index1 < 9; index1++) {
     if (a < 10)
      {
        this.urlYear = this.httpBaseYear.concat('2020', '-0', String(a), '-01?base=', this.listpays[index1], '&symbols=GBP');
        fetch(this.urlYear)
          .then(response => {return response.json();
          })
          .then(data => {
            this.barChartData[index1].data[index] = data.rates.GBP;
         });


     }
      else{
        this.urlYear = this.httpBaseYear.concat('2020', '-', String(a), '-01?base=', this.listpays[index1], '&symbols=GBP');
        fetch(this.urlYear)
         .then(response => {return response.json();
           })
          .then(data => {
