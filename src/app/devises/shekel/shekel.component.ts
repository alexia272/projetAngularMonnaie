...
export class ShekelComponent implements OnInit {
  ...
  httpDateYear = 'latest';
  listpays = ['USD', 'GBP', 'JPY', 'EUR', 'CAD', 'CHF', 'MXN', 'RUB', 'CNY'];
...
get12value(): void{
  ...
  for (let index = 0; index < 12; index++)
  {
    for (let index1 = 0; index1 < 9; index1++) {
     if (a < 10)
      {
        this.urlYear = this.httpBaseYear.concat('2020', '-0', String(a), '-01?base=', this.listpays[index1], '&symbols=ILS');
        fetch(this.urlYear)
          .then(response => {return response.json();
          })
          .then(data => {
            this.barChartData[index1].data[index] = data.rates.ILS;
         });


     }
      else{
        this.urlYear = this.httpBaseYear.concat('2020', '-', String(a), '-01?base=', this.listpays[index1], '&symbols=ILS');
        fetch(this.urlYear)
         .then(response => {return response.json();
           })
          .then(data => {
