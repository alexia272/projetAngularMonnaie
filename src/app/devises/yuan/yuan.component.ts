...
export class YuanComponent implements OnInit {
  ...
  httpDateYear = 'latest';
  listpays = ['USD', 'GBP', 'JPY', 'ILS', 'CAD', 'CHF', 'MXN', 'RUB', 'EUR'];
  ...
  lien(e) {
    ...
    switch(e.target.value){
      case'1':
      this.url = this.httpBase.concat('base=CNY&symbols=USD');
      this.nom = "Le Dollar-USA";
      this.symbol = " $";
      break;
      case'2':
      this.url = this.httpBase.concat('base=CNY&symbols=GBP');
      this.nom = "La livre";
      this.symbol = " £";
      break;
      case'3':
      this.url = this.httpBase.concat('base=CNY&symbols=CAD');
      this.nom = "Le Dollar-Canadien";
      this.symbol = " $C";
      break;
      case'4':
      this.url = this.httpBase.concat('base=CNY&symbols=ILS');
      this.nom = "Le Shekel";
      this.symbol = " ₪";
      break;
      case'5':
      this.url = this.httpBase.concat('base=CNY&symbols=EUR');
      this.nom = "L'Euro'";
      this.symbol = " €";
      break;
      case'6':
      this.url = this.httpBase.concat('base=CNY&symbols=CHF');
      this.nom = "Le Franc-Suisse";
      this.symbol = " CHF";
      break;
      case'7':
      this.url = this.httpBase.concat('base=CNY&symbols=MXN');
      this.nom = "Le Peso";
      this.symbol = " MXN";
      break;
      case'8':
      this.url = this.httpBase.concat('base=CNY&symbols=RUB');
      this.nom = "Le Rouble";
      this.symbol = " ₽";
      break;
      case'9':
      this.url = this.httpBase.concat('base=CNY&symbols=JPY');
      this.nom = "Le Yen";
      this.symbol = " ¥";
      break;
     }
...
get12value(): void{
  ...
  for (let index = 0; index < 12; index++)
  {
    for (let index1 = 0; index1 < 9; index1++) {
     if (a < 10)
      {
        this.urlYear = this.httpBaseYear.concat('2020', '-0', String(a), '-01?base=', this.listpays[index1], '&symbols=CNY');
        fetch(this.urlYear)
          .then(response => {return response.json();
          })
          .then(data => {
            this.barChartData[index1].data[index] = data.rates.CNY;
         });


     }
      else{
        this.urlYear = this.httpBaseYear.concat('2020', '-', String(a), '-01?base=', this.listpays[index1], '&symbols=CNY');
        fetch(this.urlYear)
         .then(response => {return response.json();
           })
          .then(data => {
